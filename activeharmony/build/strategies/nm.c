...
#ifdef SHSONG
simplex_init_t init_method  = SIMPLEX_INIT_USER;
#else
...
#ifdef SHSONG
    case SIMPLEX_INIT_USER:       init_by_user(); break;
#endif
    default:
...
{
    int i, j;
    const char *user_vertex_file;
    FILE *f;
    user_vertex_file = session_query("SHSONG_USER_VERTEX_FILE");
    //user_vertex_file = hcfg_get(sess->cfg, "SHSONG_USER_VERTEX_FILE");
    //user_vertex_file = "/hivehomes/shsong/user-vertex";
    f = fopen(user_vertex_file, "r");
    for (i = 0; i < simplex_size; ++i) {
        vertex_t *v;
        v = vertex_alloc();
        // read vertex i from file
        //for (j = 0; j < sess->sig.range_len; j++) {
        for (j = 0; j < simplex_size - 1; j++) {
            long c;
            fscanf(f, "%ld", &c);
            //while (fgets(buf, sizeof(buf), f) != NULL)
            // read coord j from file
            //simplex->vertex[i]->term[j] = c;
            base->vertex[i]->term[j] = c;
        }
    }
    fclose(f);
    return 0;
